...
async fn run_info(rpc_url: String) -> Result<(), Box<dyn std::error::Error>> {
  ...
  let address: web3::types::Address =
    convert_or_err(response_dto.address.as_ref(), "empty address")?;
...
fn format_balance(entry: &BalanceEntry) -> Result<String, Box<dyn Error>> {
  let token_address: web3::types::Address =
    convert_or_err(entry.token.as_ref(), "missing token address")?;
  let amount: web3::types::U256 =
    convert_or_err(entry.available.as_ref(), "missing available amount")?;
